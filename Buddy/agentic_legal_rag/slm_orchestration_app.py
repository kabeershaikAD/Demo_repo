...
class SLMOrchestrationApp:
    ...
    async def process_query(self, query: str) -> Dict[str, Any]:
        ...
        # Get verification score from result if available
        verification_score = result.get("confidence", 0.5)  # This comes from verification
        slm_confidence = analysis.get('confidence', 0.7)
        doc_count = len(result.get("documents", []))
        doc_confidence = min(doc_count / 5.0, 1.0)  # Normalize based on document count
        
        # Weighted confidence: 40% SLM orchestration confidence, 40% verification, 20% document availability
        overall_confidence = (slm_confidence * 0.4) + (verification_score * 0.4) + (doc_confidence * 0.2)
    ...
    async def _execute_agent_workflow(self, query: str, agent_sequence: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence": context.get("verification_score", 0.0),
            "agent_sequence": agent_sequence
        }
